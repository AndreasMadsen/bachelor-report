...
If one sets $\mathbf{v}_{w_O} = \mathbf{W}_O \mathbf{x}_{w_O}$ and $\mathbf{v}_{w_I} = \mathbf{z}_{w_I}$, the expression becomes the same as in \cite[eq. 2]{word2vec-details}: \todo{can this really be true. If so, it is a very confusion notation.}
\begin{equation}
p(w_O | w_I) = \frac{
