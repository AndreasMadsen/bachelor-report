...
One reason for \textit{stochastic gradient descent} being better is that it handles redundancies\todo{Why was this} in the data better. It also helps in not getting stuck in a local minima, because it is unlikely that what is a local minima for one data point is also a local minima for another data point \cite{bishop}.
